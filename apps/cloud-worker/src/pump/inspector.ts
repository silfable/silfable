...
const MANIFESTS: Record<string, { programId: string; instructionName: PumpInspectionResult["instructionName"]; discriminator: number[]; roles: Role[] }> = {
  "pump:buy": { programId: PUMP_PROGRAM_ID, instructionName: "buy_exact_quote_in_v2", discriminator: [194, 171, 28, 70, 104, 77, 91, 47], roles: PUMP_BUY },
  "pump:sell": { programId: PUMP_PROGRAM_ID, instructionName: "sell_v2", discriminator: [93, 246, 130, 60, 231, 233, 64, 178], roles: PUMP_SELL },
};

export function pumpInstructionManifest(venue: Venue, side: Side) {
  const manifest = MANIFESTS[`${venue}:${side}`];
  if (!manifest) throw new Error(`Unsupported venue/side: ${venue}:${side}`);
  return {
    idlRevision: PUMP_IDL_REVISION,
    programId: manifest.programId,
    instructionName: manifest.instructionName,
    discriminator: [...manifest.discriminator],
    roles: manifest.roles.map((expected: Role) => ({ ...expected })),
  };
}
...
export function inspectPumpInstructionPlan(plan: PumpInstructionPlan): PumpInspectionResult {
  ...
  if (!manifest) throw new Error(`Unsupported venue/side: ${plan.venue}:${plan.side}`);
  ...
  const roleShape = plan.accounts.length === manifest.roles.length && manifest.roles.every((expected: Role, index: number) => {
