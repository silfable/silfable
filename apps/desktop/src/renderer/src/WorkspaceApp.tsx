 ...
 const [preparingBridgeIds, setPreparingBridgeIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      window.silfable.checkForUpdates().then((response) => {
        const update = response.update;
        if (!activeRequest || !response.ok || !update?.available) return;
        if (window.localStorage.getItem("silfable:update-remind-later") === update.latestVersion) return;
   ...
   <main className="workspace" data-theme="dark">
      {desktopUpdate ? (
        <aside className="desktopUpdateNotice" role="status" aria-live="polite">
          <div>
            <span>Atlas update / {desktopUpdate.latestVersion}</span>
            <strong>A new Silfable desktop release is available.</strong>
          ...
          <div className="desktopUpdateActions">
            <button type="button" onClick={() => void window.silfable.openUpdateRelease(desktopUpdate.releaseUrl)}>View changes</button>
            <button type="button" className="primary" onClick={() => void window.silfable.openUpdateRelease(desktopUpdate.releaseUrl)}>Download update</button>
            <button type="button" aria-label="Remind me later" onClick={() => {
              window.localStorage.setItem("silfable:update-remind-later", desktopUpdate.latestVersion);
