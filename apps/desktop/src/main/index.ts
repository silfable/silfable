...
import { checkLatestRelease, SILFABLE_RELEASES_URL } from "./update/release-check.js";
...
function resolveEvmTokenMetadata(address: string): { symbol: string; decimals: number } {
  ...
  ipcMain.handle("update:open-release", async (_event, releaseUrl?: unknown) => {
    const url = typeof releaseUrl === "string" && releaseUrl.startsWith("https://github.com/silfable/silfable/releases/") ? releaseUrl : SILFABLE_RELEASES_URL;
