...
export function calculatePumpFeePreview(input: {
  ...
  const virtualToken = positive(input.evidence.curve.virtualTokenReserves, "virtual token reserves");
  const virtualQuote = positive(input.evidence.curve.virtualQuoteReserves, "virtual quote reserves");
  const supply = input.evidence.curve.mayhemMode
    ? positive(input.evidence.curve.tokenTotalSupply, "token total supply")
  ...
  if (input.side === "buy") {
    ...
    const realToken = positive(input.evidence.curve.realTokenReserves, "real token reserves");
  ...
  return {
    ...
    networkFeeLamports: null,
    rentLamports: null,
    disclosure: "Protocol and creator fees are calculated from finalized Pump state using the pinned SDK formula. Buyback is shown as an allocation setting, not added as an unverified extra fee. Network fee and rent remain unavailable until unsigned transaction simulation.",
  };
}

function selectFeeTier(evidence: PumpV2FinalizedBuildEvidence, marketCap: bigint) {
