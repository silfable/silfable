...
import { CreateEvmAutomationSchema, encodeStoredEvmToken } from "@/lib/evm-automation-core";
...
export async function createOwnedEvmAutomation(input: { userId: string; request: unknown }) {
  ...
  const strategy = await cloudDb.evmAutomationStrategy.create({
    data: {
      ...
      inputToken: encodeStoredEvmToken(strategyInput.common.input), inputDecimals: strategyInput.common.input.decimals,
      outputToken: encodeStoredEvmToken(strategyInput.common.output), outputDecimals: strategyInput.common.output.decimals,
