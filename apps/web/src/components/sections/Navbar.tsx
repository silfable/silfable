  ...
  return (
    <header className="publicNav fixed inset-x-0 top-0 z-50">
      <div className="section-shell publicNavInner flex items-center justify-between gap-5">
        ...
        <nav aria-label="Primary navigation" className="atlasNavLinks hidden items-center lg:flex">
          {navLinks.map(link => <Link key={link.label} href={link.href} className="atlasNavLink">{link.label}</Link>)}
          <a href={SILFABLE_X_URL} target="_blank" rel="noopener noreferrer" className="atlasNavLink atlasSocialLink" aria-label="Follow Silfable on X"><XMark /></a>
        </nav>
